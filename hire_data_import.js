    ...
    `;
        ...
        for (const [index, row] of rows.entries()) {
            ...
            try {
                ...
                // let logSql = insertSql;
                // Object.entries(params).forEach(([key, value]) => {
                //     const paramValue = value === null ? 'NULL' : `'${value}'`;
                //     logSql = logSql.replace(new RegExp(`@${key}`, 'g'), paramValue);
                // });
                // console.log(`\nExecuting SQL for row ${index + 1}:`);
                // console.log(logSql);
                ...
                if (result.rowsAffected[0] > 0) {
                    ...
                    console.log(`Row ${index + 1} skipped (duplicate): EmployerID=${params.EmployerID}, IANumber=${params.IANumber}, StartDate=${params.StartDate}`);
                }
            } catch (err) {
                failCount++;
