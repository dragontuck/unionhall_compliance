/**
 * AxiosApiClient - Concrete implementation of IApiClient using Axios
...
export class AxiosApiClient implements IApiClient {
    ...
    async getRuns(filters?: { reviewedDate?: string; modeId?: number }): Promise<ComplianceRun[]> {
    ...
    async getReportDetailsByRun(runId: number): Promise<ComplianceReportDetail[]> {
        const { data } = await this.api.get<ComplianceReportDetail[]>(`/report-details/run/${runId}`);
        return data;
    }

    async getLastHiresByRun(runId: number): Promise<ComplianceReportDetail[]> {
        const { data } = await this.api.get<ComplianceReportDetail[]>(`/last-hires/run/${runId}`);
        return data;
    }

    async getRecentHiresByRun(runId: number): Promise<RecentHireData[]> {
        const { data } = await this.api.get<RecentHireData[]>(`/recent-hires/run/${runId}`);
        return data;
    }

    async getRunReport(runId: number): Promise<ComplianceReport[]> {
        const { data } = await this.api.get<ComplianceReport[]>(`/export/report/${runId}`);
        return data;
    }
    ...
    async exportRunToExcel(runId: number): Promise<Blob> {
