...
async function generateExcelReport(pool, runId, outFile) {
  ...
  //     )
  //     SELECT Max(id) as id, ReviewedDate, ModeId FROM (
  //       SELECT s.id, s.ReviewedDate, s.ModeId FROM Segmented s CROSS JOIN LatestSegment ls
  //       WHERE s.ModeId = @modeId AND s.ModeSegment = ls.LatestSegment
  //     ) q GROUP BY q.ReviewedDate, q.ModeId`)).recordset;
